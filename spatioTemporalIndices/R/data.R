...
readRstoxFiles <- function(conf) {
  ...
  yearsAvailble = do.call(
    c,
    lapply(files, function(f) as.numeric(gsub("[^0-9]", "", f)))
  )
  yearIndex = which(yearsAvailble %in% conf$years)

  data = lapply(yearIndex, function(f) {
    getBaseline(
      files[[f]],
      startProcess = 1,
      endProcess = 10,
      parlist = list(
        RegroupLengthDist = list(
          LengthDist = "Process(StationLengthDist)",
          LengthInterval = 5
        )
      )
    )
...
setupData = function(data = NULL, conf, confPred = NULL, useProj = FALSE, ...) {
  ...
  if (is.null(confPred)) {
    # Prefer data-hull based prediction grid when user provides custom data
    confPred <- defConfPredFromData(
      data,
      utmZone = 27, ## Iceland
      nIntPoints = 4000
    )
    ...
    #   for (d in data) {
    #     if (
    #       is.data.frame(d) && all(c("UTMX", "UTMY", "x", "y") %in% names(d))
    #     ) {
    #       sample_df <- d
    #       break
    #     }
    ...
    #     lon_med <- suppressWarnings(median(
    #       as.numeric(sample_df$x),
    #       na.rm = TRUE
    #     ))
    #     lat_med <- suppressWarnings(median(
    #       as.numeric(sample_df$y),
    #       na.rm = TRUE
    #     ))
    #     utmZone <- 35
    #     if (
  ...
  if (isTRUE(confPred$useDataHull) && is.null(confPred$Strata)) {
    confPred$Strata <- 1
  }
  ...
  if (conf$cutoff[3] == 0) {
    meshS = createMesh(
      cutoff = conf$cutoff[1],
      cbound = conf$cbound[1],
      confPred = confPred
    )$mesh
    meshST = meshS
  } else {
    ...
    meshST = createMesh(
      cutoff = conf$cutoff[2],
      cbound = conf$cbound[2],
      confPred = confPred
    )$mesh
  }
  ...
  # fishObsList = vector("list", length(conf$years))
  ...
  d <-
    data |>
    arrange(LengthGroup) |>
    #select(Station, LengthGroup, fishObs) |>
    pivot_wider(
      names_from = LengthGroup,
      values_from = fishObs,
      values_fill = 0
    ) |>
    select(-`NA`) |>
  ...
  A_ListS <-
    d |>
    split(d$year) |>
    map(select, UTMX, UTMY) |>
    map(as.matrix) |>
    map(~ inla.spde.make.A(mesh = meshS, loc = .))
  ...
  A_ListST <-
    ...
    map(select, UTMX, UTMY) |>
    map(as.matrix) |>
    map(~ inla.spde.make.A(mesh = meshST, loc = .))

  d |>
    group_by(year) |>
    summarise(
      nStations = n_distinct(Station)
    ) |>
    select(nStations) |>
    unlist() -> nStationsEachYear

  distance = d$dist
  yearObs = d$year
  station = d$Station
  locObs = d |> select(UTMX, UTMY) |> as.matrix()
  locObsLatLon = d |> select(x, y) |> as.matrix()
  timeInDay = d$timeInDay
  sunAlt = d$sunAlt
  sunSetting = d$sunSetting
  date = d$date
  depth = d$depth
  ...
  #   if (
  #     is.list(data) &&
  #       !is.null(data[[year_key]]) &&
  #       is.data.frame(data[[year_key]])
  #   ) {
  #     req_cols <- c(
  #       "Station",
  #       "LengthGroup",
  ...
  #       "depth"
  #     )
  #     if (all(req_cols %in% names(data[[year_key]]))) {
  #       d <- data[[year_key]]
  #     }
  #   }
  #   if (is.null(d)) {
  ...
  #   #   dL=d[which(d$LengthGroup==l),] |> full_join(d |> select(Station) |> distinct(),by="Station") |> mutate(fishObs=ifelse(is.na(fishObs),0,fishObs))
  ...
  fishObsMatrix <-
    d |>
    dplyr::select(dplyr::all_of(as.character(conf$lengthGroups))) |>
    as.matrix()
  rownames(fishObsMatrix) = d$Station
  ...
  if (conf$sunAlt[1] < 2) {
    ...
    sunAltFormula = as.formula(paste(
      "fishObsMatrix ~ sin+cos + sin2 + cos2 + sin3 + cos3"
    ))
    sunAltFormulaIntRep = as.formula(paste(
      "sunAlt ~ sin+cos + sin2 + cos2 + sin3 + cos3"
    ))
  } else {
  ...
  #       day = dateThis[3],
  #       month = dateThis[2],
  #       lat = locObsLatLon[i, 2],
  #       lon = locObsLatLon[i, 1]
  #     )$alt.of.sun
  #     if (altMax < alt) {
  #       altMax = alt
  #     }
  ...
  sunAltTrans <- d$sunAltTrans

  gamSetup_sunAlt = mgcv::gam(
    sunAltFormula,
    data = data.frame(
      haulID = rownames(fishObsMatrix),
      sin = sin(sunAltTrans * 2 * pi),
      cos = cos(sunAltTrans * 2 * pi),
      cos2 = cos(2 * sunAltTrans * 2 * pi),
      cos3 = cos(3 * sunAltTrans * 2 * pi),
      cos4 = cos(4 * sunAltTrans * 2 * pi),
      cos5 = cos(5 * sunAltTrans * 2 * pi),
      sin2 = sin(2 * sunAltTrans * 2 * pi),
      sin3 = sin(3 * sunAltTrans * 2 * pi),
      sin4 = sin(4 * sunAltTrans * 2 * pi),
      sin5 = sin(5 * sunAltTrans * 2 * pi),
      fishObsMatrix = fishObsMatrix[, 1]
    ),
    fit = FALSE
  )

  gamSetup_depth = mgcv::gam(
    fishObsMatrix ~ s(depth, bs = "cs", k = conf$splineDepth[1]),
    data = data.frame(
      haulID = rownames(fishObsMatrix),
      depth = depth,
      fishObsMatrix = fishObsMatrix[, 1]
  ...
  gamSetup_sunAltReport = gam(
    sunAltFormulaIntRep,
    data = data.frame(
      sunAlt = seq(0, 1, by = 0.01),
      sin = sin(seq(0, 1, by = 0.01) * 2 * pi),
      sin2 = sin(seq(0, 1, by = 0.01) * 2 * 2 * pi),
      sin3 = sin(seq(0, 1, by = 0.01) * 3 * 2 * pi),
      cos = cos(seq(0, 1, by = 0.01) * 2 * pi),
      cos2 = cos(seq(0, 1, by = 0.01) * 2 * 2 * pi),
      cos3 = cos(seq(0, 1, by = 0.01) * 3 * 2 * pi)
    ),
    fit = FALSE
  )
  ...
  maxSunAlt = 0.5
  gamSetup_sunAltIntegrate = gam(
    sunAltFormulaIntRep,
    data = data.frame(
      sunAlt = rep(maxSunAlt, 2),
      sin = sin(rep(maxSunAlt, 2) * 2 * pi),
      cos = cos(rep(maxSunAlt, 2) * 2 * pi),
      cos2 = cos(2 * rep(maxSunAlt, 2) * 2 * pi),
      cos3 = cos(3 * rep(maxSunAlt, 2) * 2 * pi),
      sin2 = sin(2 * rep(maxSunAlt, 2) * 2 * pi),
      sin3 = sin(3 * rep(maxSunAlt, 2) * 2 * pi)
    ),
    fit = FALSE
  )
  X_sunAltIntegrate = gamSetup_sunAltIntegrate$X[, -1]

  X_depthReport = PredictMat(
    gamSetup_depth$smooth[[1]],
    data = data.frame(depth = as.numeric(seq(min(depth), max(depth), by = 1)))
  )
  ...
  if (isTRUE(confPred$useDataHull)) {
    ...
    hullPoly <- SpatialPolygons(
      list(Polygons(list(Polygon(hullCoords)), "1")),
      proj4string = utmCRS
    )
    areas <- hullPoly@polygons[[1]]@area * (1 / 1.852)^2
    areas <- areas[1]
  } else {
    dataDir <- system.file("extdata", package = "spatioTemporalIndices")
    strata = read.delim(
      paste0(dataDir, "/vintertokt_barentshavny.txt"),
      head = F
    )
    stratas = lapply(strata$V2, function(f) {
      as(sf::st_as_sfc(f, crs = 4326), "Spatial")
    })
    stratasUTM = lapply(stratas, function(f) {
      proj4string(f) = CRS("+proj=longlat +datum=WGS84")
      f <- spTransform(
        f,
        CRS("+proj=utm +zone=35 +datum=WGS84 +units=km +no_defs")
      )
      f
    })
...
dataInYear = function(year, conf, data) {
  ...
  for (id in unique(d$Station)) {
    ...
    if (maxOnStation > conf$maxLength) {
      ...
      d$WeightedCount[d$Station == id & d$LengthGroup == conf$maxLength] =
        sum(d$WeightedCount[
          !is.na(d$WeightedCount) &
            d$LengthGroup >= conf$maxLength &
            d$Station == id
        ])
      ...
      newData = data.frame(
        SpecCat = rep(d$SpecCat[1], NumberOfNewLengthGroups),
        Station = rep(id, NumberOfNewLengthGroups),
        LengthGroup = seq(from = maxOnStation + 5, to = conf$maxLength, by = 5),
        LengthInterval = rep(5, NumberOfNewLengthGroups),
        WeightedCount = rep(0, NumberOfNewLengthGroups),
        LengthDistType = rep(d$LengthDistType[1], NumberOfNewLengthGroups)
      )
      d = rbind(d, newData)
    }
    ...
    if (minOnStation < conf$minLength) {
      ...
      newData = data.frame(
        SpecCat = rep(d$SpecCat[1], NumberOfNewLengthGroups),
        Station = rep(id, NumberOfNewLengthGroups),
        LengthGroup = seq(from = conf$minLength, to = minOnStation - 5, by = 5),
        LengthInterval = rep(5, NumberOfNewLengthGroups),
        WeightedCount = rep(0, NumberOfNewLengthGroups),
        LengthDistType = rep(d$LengthDistType[1], NumberOfNewLengthGroups)
      )
  ...
  for (i in 1:nObs) {
    ...
    day = date[1]
    month = date[2]
    lat = dS$latitudestart[haul]
    lon = dS$longitudestart[haul]
    d$sunAlt[i] = altOfSun(
      min = min,
      hour = hour,
      day = day,
    ...
    if (!is.na(dS$fishingdepthmax[haul]) & !is.na(dS$fishingdepthmin[haul])) {
      ...
      !is.na(dS$fishingdepthmax[haul]) & is.na(dS$fishingdepthmin[haul])
    ) {
...
includeIntPoints <- function(data, conf, confPred) {
  ...
  ApredS = inla.spde.make.A(
    attributes(data)$meshS,
    loc = as.matrix(points$locUTM)
  )
  ApredST = inla.spde.make.A(
    attributes(data)$meshST,
    loc = as.matrix(points$locUTM)
  )
  ...
  #Find depth covariate using nearest observed point (no rgeos dependency)
  utmZone <- if (!is.null(confPred$utmZone)) confPred$utmZone else 35
  utmCRS <- CRS(paste0(
    "+proj=utm +zone=",
    utmZone,
  ...
  obs_df <- data.frame(
    x = attributes(data)$locObs[, 1],
    y = attributes(data)$locObs[, 2]
  )
  int_df <- data.frame(x = points$locUTM[, 1], y = points$locUTM[, 2])
  obs_sf <- sf::st_as_sf(obs_df, coords = c("x", "y"), crs = utmCRS)
  int_sf <- sf::st_as_sf(int_df, coords = c("x", "y"), crs = utmCRS)
  nearest_idx <- sf::st_nearest_feature(int_sf, obs_sf)

  xDepth = matrix(0, dim(points$locUTM)[1], dim(data$X_depth)[2])
  xDepth = data$X_depth[nearest_idx, ]
  data$X_depth_int = xDepth

  #Set sun altitude given at maximum
  X_sunAltIntegrateTmp = attributes(data)$X_sunAltIntegrate
  X_sunAltIntegrate = matrix(
    0,
    dim(points$locUTM)[1],
    dim(X_sunAltIntegrateTmp)[2]
  )
  for (i in 1:dim(points$locUTM)[1]) {
    X_sunAltIntegrate[i, 1:dim(X_sunAltIntegrateTmp)[2]] = X_sunAltIntegrateTmp[
      1,
      1:dim(X_sunAltIntegrateTmp)[2]
    ]
  }
