...
fitModel <- function(
  ...
  obj <- TMB::MakeADFun(
    data = data,
    parameters = par,
    random = c("xS", "xST", "betaDepth", "nugget", "nuggetIndex"),
    profile = c("beta0", "betaSun"),
    map = map,
    DLL = "spatioTemporalIndices",
    silent = TRUE
  )
  ...
  for (nn in names(low)) {
    lower[names(obj$par) == nn] = low[[nn]]
  }
  for (nn in names(up)) {
    upper[names(obj$par) == nn] = up[[nn]]
  }
...
jit <- function(run, njit, ncores = 1, sd = 0.2) {
  ...
  par <- lapply(1:njit, function(i) {
    relist(pOriginal + rnorm(length(pOriginal), sd = sd), run$par)
  })
  ...
  par = lapply(par, function(p) {
    for (i in 1:length(run$map)) {
      ...
      p[[j]][which(is.na(run$map[[i]]))] = parOriginal[[
        j
      ]][which(is.na(run$map[[i]]))]
    }
    p
  })

  if (ncores == 1) {
    runs = lapply(par, function(f) {
      fitModel(data = run$data, par = f, conf = run$conf, map = run$map)
    })
  } else {
    ...
    runs = parLapply(cl, par, function(f) {
      fitModel(data = run$data, par = f, conf = run$conf, map = run$map)
    })
    stopCluster(cl)
  }
